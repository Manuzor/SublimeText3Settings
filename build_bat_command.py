# Note: This is a modified version of the Default package's exec.py ()
#  - Manuel Maier <mjmaier@gmx.de>, 2019-02-03
...
# Parameter `start_path` may be a file or a directory. It must be an absolute path though.
def find_build_bat(start_path):
    prev_dir = None
    dir_name = start_path
    while dir_name != prev_dir:
        # print("Searching in:", dir_name)
        candidate = os.path.join(dir_name, "build.bat")
...
class BuildBatCommand(sublime_plugin.WindowCommand, ProcessListener):
    BLOCK_SIZE = 2**14
    ...
    def run(
        ...
        bat_file = find_build_bat(self.window.active_view().file_name())
        if not bat_file:
            sublime.status_message("Unable to find build.bat")
            return
        shell_cmd = bat_file
